...
impl V3Decryptor {
    ...
    async fn decrypt_database_impl(
        ...
        info!("开始V3数据库解密: {:?} -> {:?}", input_path, output_path);
        ...
        info!("V3数据库解密完成，处理了 {} 页", processed_pages);
        Ok(())
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_v3_decryptor_creation() {
        let decryptor = V3Decryptor::new();
        assert_eq!(decryptor.version(), DecryptVersion::V3);
        assert_eq!(decryptor.config().iter_count, 64000);
        assert_eq!(decryptor.config().hmac_size, 20);
    }
